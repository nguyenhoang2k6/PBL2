...
bool NV_NewPass::Init() {
    ...
    label_newpass = new Label("Đổi mật khẩu", COLOR_WARNING, 1100.0f, 400.0f, font3, renderer);
...
void NV_NewPass::handleEvent(const SDL_Event& e) {
    ...
    if(button_save->isClicked()) {
        ...
        ResetStatusNV result = resetPassNV(currentUsername, oldPassInput, newPass1, newPass2);
        switch (result)
        {
        case RESET_SUCCESS_NV:
            error1 = false;
            error2 = false;
            success = true;
            break;
        case INCORRECT_CURRENT_PASS_NV:
            error1 = true;
            error2 = false;
            success = false;
            break;
        case NEW_PASSWORDS_MISMATCH_NV:
            error1 = false;
            error2 = true;
            success = false;
            break;
        case USER_NOT_FOUND_NV:
            std::cerr << "Lỗi: Người dùng không tồn tại!" << std::endl;
            break;
        default:
            break;
        }
